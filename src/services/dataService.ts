...
let tomor: Date = new Date('08 29 2018');

let users: User[] = [
    { id: 1, firstName: "Ivan", lastName: "Ivanov", description: "-", birthDate: new Date('01 01 1981'), employementDate: new Date('10 06 2014'), workingStartDate: new Date('11 09 2003') },
    { id: 2, firstName: "Igor", lastName: "Surkov", description: "-", birthDate: new Date('11 09 1974'), employementDate: new Date('04 01 2010'), workingStartDate: new Date('12 12 1998') },
]
...
let userSpecializations: UserSpecialization[] = [
...
let appointments: Appointment[] = [
    { id: 1, userId: 1, patientId: 1, date: today, admissionTime: { startTime: "10:00", endTime: "10:30" }, specialization: Specialization.Dermatology, isCancelled: false },
    { id: 2, userId: 1, patientId: 2, date: today, admissionTime: { startTime: "11:00", endTime: "12:00" }, specialization: Specialization.Dermatology, isCancelled: false },
    { id: 3, userId: 1, patientId: 3, date: today, admissionTime: { startTime: "12:00", endTime: "12:30" }, specialization: Specialization.Dermatology, isCancelled: true  },
    { id: 4, userId: 1, patientId: 4, date: today, admissionTime: { startTime: "14:00", endTime: "14:30" }, specialization: Specialization.Oncology,    isCancelled: false },
    { id: 5, userId: 2, patientId: 1, date: today, admissionTime: { startTime: "09:00", endTime: "10:00" }, specialization: Specialization.Surgery,     isCancelled: false },
    { id: 6, userId: 2, patientId: 2, date: today, admissionTime: { startTime: "10:00", endTime: "10:30" }, specialization: Specialization.Surgery,     isCancelled: false },
    { id: 7, userId: 2, patientId: 3, date: tomor, admissionTime: { startTime: "12:00", endTime: "13:00" }, specialization: Specialization.Surgery,     isCancelled: false },
    { id: 8, userId: 2, patientId: 4, date: today, admissionTime: { startTime: "15:00", endTime: "16:00" }, specialization: Specialization.Therapy,     isCancelled: false }
]

export default class DataService {
    public getUser = (id: number): User => {
        let user: User = users.find((u: User) => u.id === id);
        return user ? user : users[0];
    }

    public getPatient = (id: number): Patient => {
        let patient: Patient = patients.find((p: Patient) => p.id === id);
        return patient ? patient : patients[0];
    }

    public getUserCurrentTitle = (id: number): Title => {
        let titles: Title[] = userTitles
    ...
    public getAppointments = (userId: number, date: Date): Appointment[] => {
        let begin: Date = moment(date).startOf("day").toDate();
        let end: Date = moment(date).endOf("day").toDate();
